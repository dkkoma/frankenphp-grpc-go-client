...
`UnaryCall` references a PHP `Channel` object and a method path without allocating Go-side call state during construction. `start()` creates a context, applies outgoing metadata and optional deadline, invokes grpc-go with raw request and response byte buffers, stores the last peer on the PHP object, then returns `UnaryResult`.
