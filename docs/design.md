...
`Channel` maps to one grpc-go `ClientConn`. It owns target, dial options, close state, and a small set of option conversions. `close()` is idempotent. Calls started after close throw a PHP exception. Calls already in flight rely on grpc-go behavior and must not crash PHP.
